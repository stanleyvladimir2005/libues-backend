...
public class ProductController {
    ...
    public ResponseEntity<List<ProductDTO>> findAll(){
        val product = service.findAll().stream().map(this::convertToDto).collect(Collectors.toList());
    ...
    public ResponseEntity<Object> save(@Valid @RequestBody ProductDTO productDTO) {
        val product = service.save(convertToEntity(productDTO));
        val location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(product.getIdProduct()).toUri();
        return ResponseEntity.created(location).build();
    }

    @PutMapping(consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Object> update(@Valid @RequestBody ProductDTO productDTO) {
        val product = service.findById(productDTO.idProduct());
        if (product == null)
    ...
    public ResponseEntity<Page<ProductDTO>> listPageable(Pageable pageable) {
        val productDTO = service.listPageable(pageable).map(this::convertToDto);
